...
void link_edge(int u) {
    ...
    sort(nek_A[u].begin(), nek_A[u].end(), [&](int x, int y) { return rel_a[x].len < rel_a[y].len; });
    sort(nek_B[u].begin(), nek_B[u].end(), [&](int x, int y) { return rel_b[x - na].len < rel_b[y - na].len; });
