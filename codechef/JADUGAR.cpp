...
void init(int n, int k, int a, int b) {
    int T = 30;
    f[1] = k;
    for(int i = 2 ; i <= T ; ++ i) {
        f[i] = (ll) a * f[i - 1] % mod;
        for(int j = 1 ; j < i ; ++ j) {
            upd(f[i], (ll) b * f[j] % mod * f[i - j] % mod);
        }
    }
    for(int i = 0 ; i <= T ; ++ i) {
        f[i] = f[i + 1];
...
//        } puts("");
    ...
    int START = 3, END = START + (L + 1) * (DEG + 1) + 2;
    
    for(int n = START ; n <= END ; ++ n) {
        vector<ll> coef;
        for(int i = 0 ; i <= L ; ++ i) {
            for(int j = 0 ; j <= DEG ; ++ j) {
                coef.push_back(pw(n - i, j) * f[n - i] % mod);
            }
        }
...
//    }
    ...
    if(1) {
        ...
        for(int n = START ; n <= 25 ; ++ n) {
            ll s = 0;
            for(int i = 0 ; i <= L ; ++ i) {
                for(int j = 0 ; j <= DEG ; ++ j) {
                    s += pw(n - i, j) * res[i * (DEG + 1) + j][res[0].size() - 1] % mod * f[n - i] % mod;
                    s %= mod;
                }
            }
            s = (s % mod + mod) % mod;
