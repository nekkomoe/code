...
const int _sin[10] = { 0, 1, 0, -1  };
const int _cos[10] = { 1, 0, -1, 0  };
...
int main() {
    ...
    for(int i = 0 ; i < n ; ++ i) {
        a[i] = _cos[i % 4] * invfac[i];
    }
    sol(n);

    for(int i = 0 ; i < n ; ++ i) fafa[i] = _sin[i % 4] * invfac[i];

    int len = 1; while(len <= 2 * n) len <<= 1;
    n = len;
    ntt(b, n, 0), ntt(fafa, n, 0);
    for(int i = 0 ; i < n ; ++ i) b[i] = b[i] * fafa[i] % mod;
    ntt(b, n, 1);
